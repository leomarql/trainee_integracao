...
class AStarPlanner:
    ...
    # FunÃ§Ã£o para converter coordenadas da grade para o mundo real
    ...
    # FunÃ§Ã£o para converter coordenadas do mundo real para a grade
    def world_to_grid(self, x, y):
        world_x = int((x - self.map_origin_x) / self.map_resolution)
        world_y = int((y - self.map_origin_y) / self.map_resolution)
        return world_x, world_y

    # FunÃ§Ã£o de callback para receber o occupancy grid
    ...
    # FunÃ§Ã£o para verificar se uma cÃ©lula estÃ¡ livre (valor < 50)
    ...
    # FunÃ§Ã£o para obter os vizinhos de um nÃ³
    def get_neighbors(self, node):
        ...
        for move in possible_moves:
            ...
            if self.is_cell_free(nx, ny):
                g_cost = node.g_cost + 1  # Custo do movimento para uma cÃ©lula vizinha
    ...
    # FunÃ§Ã£o para encontrar o caminho utilizando o algoritmo A*
    def find_path(self, rviz=None):
        ...
        # Criar o nÃ³ inicial e adicionar Ã  lista aberta
        ...
        past_node = None
        while open_list:
            # Obter o nÃ³ com o menor custo da lista aberta
            _, current_node = open_list.get()

            # Verificar se chegamos ao nÃ³ objetivo
            ...
            # Adicionar o nÃ³ atual Ã  lista fechada
            ...
            # Obter os vizinhos do nÃ³ atual
            neighbors = self.get_neighbors(current_node)

            for neighbor in neighbors:
                # Verificar se o vizinho jÃ¡ estÃ¡ na lista fechada
                if (neighbor.x, neighbor.y) in closed_list:
                    continue

                # Verificar se o vizinho jÃ¡ estÃ¡ na lista aberta
    ...
    #     start_node = Node(start_x, start_y, 0, self.heuristic(start_x, start_y))
    #     open_list.put((start_node.f_cost(), -start_node.h_cost, start_node))  # Adicionando -h_cost como segundo critério de ordenação
    ...
    #             for _, _, open_node in open_list.queue:
    ...
    #             open_list.put((neighbor.f_cost(), -neighbor.h_cost, neighbor))  # Adicionando -h_cost como segundo critério de ordenação

    #     rospy.loginfo("Path not found.")

    def publish_path(self, goal_node, rviz=None):
        # Percorrer o caminho a partir do nÃ³ objetivo
