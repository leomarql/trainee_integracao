...
class AStarPlanner:
    ...
    def get_neighbors(self, node):
        ...
        # Movimentos possí­veis: cima, baixo, esquerda, direita
        possible_moves = [(x, y - 1), (x, y + 1), (x - 1, y), (x + 1, y)]

        for move in possible_moves:
            nx, ny = move # (nx, ny) posicao da  celula livre
        ...
        while open_list:
            ...
            for neighbor in neighbors:
                ...
                if any(node[1].x == neighbor.x and node[1].y == neighbor.y for node in open_list.queue):
                    if any(neighbor.h_cost < node[1].h_cost for node in open_list.queue):
