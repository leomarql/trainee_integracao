...
from beginner_tutorials.srv import VelControl, VelControlResponse
...
class TurtleBot3Control:
    ...
    def handle_vel_control(self, req):
        self.target_x = req.x
        self.target_y = req.y
        self.move_turtlebot3()
        return VelControlResponse(True)
   
    def vel_control_server(self):
        s = rospy.Service('vel_control', VelControl, self.handle_vel_control)
        print("Ready to receive target position.")
        rospy.spin()
